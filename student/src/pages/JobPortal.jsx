...
import { MapPin, Briefcase, IndianRupee, ArrowRight, TrendingUp, Users, Award, Lock } from 'lucide-react';
...
const JobPortal = () => {
    ...
    useEffect(() => {
        const fetchJobs = async () => {
            try {
                ...
                console.log("Fetching jobs from strict endpoint:", `${API_URL}/api/jobs/fetch/student`);
                // Fetch ONLY student jobs using the new strict endpoint
                const res = await axios.get(`${API_URL}/api/jobs/fetch/student`);
    ...
    const closeDetailsModal = (action) => {
        ...
        if (action === 'apply') {
            if (!completionStats.enrolled) {
                toast.error("You must be enrolled in a course to apply.");
                return;
            }
            if (completionStats.completionPercentage < 75) {
                toast.error(`You need 75% course completion to apply. Current: ${completionStats.completionPercentage}%`, {
                    duration: 4000,
                    icon: '🔒'
                });
                return;
    ...
    return (
        <div className="pb-20 font-sans">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                ...
                {/* Progress Indicator */}
                <div className={`flex items-center gap-3 px-4 py-2 rounded-xl border ${completionStats.completionPercentage >= 75 ? 'bg-green-50 border-green-100 text-green-700' : 'bg-orange-50 border-orange-100 text-orange-700'}`}>
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white ${completionStats.completionPercentage >= 75 ? 'bg-green-500' : 'bg-orange-500'}`}>
                        {completionStats.completionPercentage >= 75 ? <Award size={20} /> : <Lock size={18} />}
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {loading ? (
                        ...
                        <motion.div
                            ...
                            className="bg-white rounded-2xl shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300 overflow-hidden flex flex-col h-full group border border-gray-100 hover:border-indigo-100"
                        >
