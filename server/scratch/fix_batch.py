...
with open('d:/Projects/Finwise/admin/src/pages/BatchStudents.jsx', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Update assignStudentId state to selectedStudentIds array
content = re.sub(
    r"const \[assignStudentId, setAssignStudentId\] = useState\(''\);",
    r"const [selectedStudentIds, setSelectedStudentIds] = useState([]);",
    content
)

# 2. Update handleAssignStudent
...
# 3. Update assignModalFiltered
old_filter = """    // Filtered students for assignment modal
...
new_filter = """    // Filtered students for assignment modal
    const assignModalFiltered = allStudents.filter(s =>
...
# 4. Update the onClick for the Assign Student button
...
# 5. Update the modal render (from map(s => radio) to map(s => checkbox) and add Select All)
...
new_modal = """                            <div className="overflow-y-hidden flex-1 flex flex-col rounded-lg border border-gray-200">
                                {assignModalFiltered.length === 0 ? (
                                    <div className="py-8 text-center text-gray-400 text-sm">No available students found</div>
                                ) : (
                                    <>
...
with open('d:/Projects/Finwise/admin/src/pages/BatchStudents.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
